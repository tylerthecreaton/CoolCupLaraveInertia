...
export default function ClientPage() {
    ...
    useEffect(() => {
        // Keep drawer open when items are in cart
        if (localState.cart.items?.length > 0 && !isCartExpanded) {
    ...
    return (
        <div className="flex-layout-container">
            {/* Main Content Area - Will resize when drawer opens */}
            <div
                ref={mainContentRef}
                className={`main-content ${isCartExpanded ? 'drawer-open' : ''}`}
            >
                <StorefrontLayout>
                    <Head title="ClientPage" />
                    <MainContent />
                    <div className="bg-gradient-to-b from-blue-50 to-white client-page">
                        {/* Hero Section */}
            ...
            {/* Cart Items - Flex Drawer */}
            {localState.cart.items?.length > 0 && (
                <div
                    ref={cartDrawerRef}
                    className={`drawer-container ${isCartExpanded ? 'open' : ''}`}
                >
                    <div className="relative h-full overflow-hidden">
                        ...
                        {/* Drawer Content */}
                        <div className="drawer-content flex flex-col justify-between h-full">
                            <div className="px-4">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-2xl font-semibold text-gray-800">
                                        รายการสินค้า
